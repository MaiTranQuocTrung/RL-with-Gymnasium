...
def _worker(
    ...
    action_high = torch.tensor(env.action_space.high[0], dtype=torch.float32)
    ...
    for i in range(self_train_itr):
        ...
        while not (terminated or truncated or env_n_steps >= n_steps):
            with torch.no_grad():
                ...
                raw_mean, std = local_model_policy(torch.tensor(state, dtype=torch.float32))
                scaled_mean = torch.tanh(raw_mean) * action_high
                # create Gaussian
                dist = torch.distributions.Normal(loc=scaled_mean, scale=std)
        ...
        for ((state, action, _, _, _), n_steps_return), adv in zip(zip(episode, n_steps_collection), advantages):
            raw_mean, std = local_model_policy(torch.tensor(state, dtype=torch.float32))
            scaled_means = torch.tanh(raw_mean) * action_high
            dist = torch.distributions.Normal(loc=scaled_means, scale=std)
            ...
            total_loss_value += torch.nn.functional.smooth_l1_loss(value_pred, target_tensor)
        ...
        for lp, gp in zip(local_model_policy.parameters(), model_policy.parameters()):
            if lp.grad is not None:
                gp._grad = lp.grad.clone()  # Use _grad to inject into optimizer safely
...
def train_model(
    ...
    if solved_event.is_set():
        torch.save(global_net_policy.state_dict(), f"solved models/{model_str}_{env_id}_A3C_solved.pth")
