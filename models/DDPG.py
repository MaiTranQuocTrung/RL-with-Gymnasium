...
def train_model(
                ...
                model_learning_rate: float = 0.0003,
                gamma: float = 0.99,
    ...
    loss_fn = nn.MSELoss()
    actor_optimizer = torch.optim.Adam(model_actor.parameters(), lr=model_learning_rate)
    critic_optimizer = torch.optim.Adam(model_critic.parameters(), lr=model_learning_rate)
    ...
    for i in range (self_train_itr):
        ...
        while not (terminated or truncated):
            ...
            if len(replay_buffer) > min_sample:
                ...
                # Target Q-values
                with torch.no_grad():
                    # We are trying to build Q(s',a')
                    next_actions = target_actor(next_states)
                    # now we get Q(s',a')
                    q_next = target_critic(next_states, next_actions).squeeze(-1)
                    # build the target
        ...
        if verbose and i % test_freq == 0:
            ...
            if avg_reward_per_episodes >= solved_threshold:
                ...
                torch.save(model_actor.state_dict(), f"solved models/{env_id}_DDPG_solved.pth")
